...
btn.addEventListener('click',(e)=>{
  ...
  if(isNaN(value) || value < 0 ){ // in case of not entering number
    const rand = Math.floor(Math.random()*text.length +1);
    for (let i = 0; i < rand; i++) {
      loremText.innerHTML+= `<p class="result">${text[i]}</p>`;
    }
  } else { // in case of entering number
    for (let i = 0; i < amount.value; i++) {
      const rand = Math.floor(Math.random()*text.length);
      loremText.innerHTML+= `<p class="result">${text[rand]}</p>`;
    }
  }
})